    ...
    public _onUploadFileInDevice(imageURI: string, imageFileName: string, type: any, resize: string, key?: string): Promise<any> {
        ...
        let options: FileUploadOptions = {
            ...
            params: {
                ...
                "__resize": resize,
                "__key": key ? key : "",
                "__command": "upload_image"
            }
        };

        return new Promise((resolve, reject) => {
            fileTransferObject.upload(imageURI, "http://" + ShipUgfSFSConnector.getInstance().getSFSHost() + ":" + ShipUgfSFSConnector.getInstance().getSFSPort() + "/BlueBox/SFS2XFileUpload?sessHashId=" + ShipUgfSFSConnector.getInstance().getSessionToken(), options).then(data => {
                resolve(data);
    ...
    public _onUploadFileInBrowser(selectedFile: any, type: any, resize: string, key?: string): Promise<any> {
        return new Promise((resolve, reject) => {
            if (selectedFile) {
                ...
                httpParams = httpParams.append('sessHashId', ShipUgfSFSConnector.getInstance().getSessionToken());
                ...
                const req = new HttpRequest('POST', "http://" + ShipUgfSFSConnector.getInstance().getSFSHost() + ":" + ShipUgfSFSConnector.getInstance().getSFSPort() + "/BlueBox/SFS2XFileUpload?sessHashId=" + ShipUgfSFSConnector.getInstance().getSessionToken(), formData, {
                    reportProgress: true,
                    responseType: 'text',
                    params: httpParams,
                    headers: headers
